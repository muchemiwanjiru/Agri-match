...
// Fetch featured tractors
$stmt = $conn->prepare("SELECT id, name AS title, description, amount AS price, image FROM tractors WHERE status='available' LIMIT 3");
$stmt->execute();
$tractors = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

// Fetch user reviews
$stmt = $conn->prepare("SELECT r.rating, r.comment, u.username 
                        FROM reviews r 
...
<head>
    ...
    <title>Admin Dashboard - Agri-Match</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    <section class="featured-listings py-5 bg-light">
        <div class="container text-center">
            <h2 class="mb-5">Featured Tractors</h2>
            <div class="row">
                <?php if ($tractors): ?>
                    <?php foreach ($tractors as $tractor): ?>
                        <div class="col-lg-4 col-md-6 mb-4">
                            <div class="card shadow-sm border-0">
                                <img src="<?php echo $tractor['image'] ? htmlspecialchars($tractor['image']) : 'images/default_image.png'; ?>" 
                                     class="card-img-top" alt="<?php echo htmlspecialchars($tractor['title']); ?>">
                                <div class="card-body">
                                    <h5 class="card-title"><?php echo htmlspecialchars($tractor['title']); ?></h5>
                                    <p class="card-text text-truncate"><?php echo htmlspecialchars(substr($tractor['description'], 0, 45)) . (strlen($tractor['description']) > 45 ? '...' : ''); ?></p>
                                    <p class="card-text"><strong>KSh <?php echo htmlspecialchars($tractor['price']); ?></strong></p>
                                    <a href="book_tractor.php?id=<?php echo $tractor['id']; ?>" class="btn btn-primary btn-sm">View Details</a>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
                <?php else: ?>
                    <p class="text-muted">No tractors available right now. Check back later!</p>
                <?php endif; ?>
            </div>
        </div>
    </section>
                        ...
                        <div class="col-lg-4">
                            <div class="card bg-light text-dark shadow-sm">
                                <div class="card-body">
                                    ...
                                    <footer class="blockquote-footer"><?php echo htmlspecialchars($review['username']); ?> 
