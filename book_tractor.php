...
// Fetch all operators
$sql = "SELECT id, age, sex, strength, skills, amount, status FROM operators"; // Explicitly select status
        ...
        <div class="operator-container">
            ...
            if ($result && $result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $status = isset($row['status']) ? $row['status'] : 'available'; // Fallback to 'available' if status is missing
                    $status_class = $status === 'available' ? 'status-available' : 'status-booked';
                    echo "<div class='operator-card'>
                            ...
                            <p><strong>Status:</strong> <span class='$status_class'>" . htmlspecialchars($status) . "</span></p>";
